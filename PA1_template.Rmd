...
Thus, the mean is **`r as.character(mean1)`** and the median is **`r as.character(median1)`**.
...
As a result, we can see that the the peak interval happens at **`r paste(as.character(nhour),as.character(nminute),sep="") `**.
...
```
 ...
 Thus, the total number of missing values is **`r as.character(nna_vals) `**. And, there are
 **`r as.character(nna_days) `** days in which missing value cases occur.
...
From this, the new mean is **`r as.character(mean2)`** and the new median is **`r as.character(median2)`**.

Comparing with the previous values, the change in mean is **`r as.character(mean2 - mean1)`** and the change in median is **`r as.character(median2 - median1)`**.
